...
public partial class AddProductPage : ContentPage
{
	...
	private async void OnSaveButtonClicked(object sender, EventArgs e)
	{
		...
		if (check.Equals(Product))
		{
			...
			try { await _dbService.SaveItemAsync<Product>(newProduct); }
			catch (Exception ex) { await DisplayAlert("Error", ex.Message, "OK"); }

			await DisplayAlert("", $"{newProduct.Cena}, {newProduct.JednostkaMiary}, {newProduct.Id}, {Product.Id}", "OK");
		}
		else
		{
			...
			try { await _dbService.SaveItemAsync<Product>(Product); }
			catch (Exception ex) { await DisplayAlert("Error", ex.Message, "OK"); }

			await DisplayAlert("", $"{Product.Cena}, {Product.JednostkaMiary}, {Product.Id}", "OK");
		}
